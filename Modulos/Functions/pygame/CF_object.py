...
from Modulos.Functions.pygame.pygame_general_function import *
import random
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # Recuperar energia solo si el jugador no hace acciones
        # Perder energia al realizar movimientos de esfuerzo
        if self.stamina_active == True:
            if (
                self.move_jump == False and
                self.move_right == False and
                self.move_left == False and
                self.hit_normal == False and
                self.hit_power == False #or
                #fall == True
            ):
                self.stamina += 1
        ...
        if hit == True:
            get_sound( 'hits-missed', None ).play()
            ...
            if hit_type[0] == 'arms':
                ...
                if hit_type[3] == 'jump_or_fall':
                    self.collide_hit.damage = self.collide_hit.damage*1.25
            ...
            # Perder energia dependiendo del golpe
            self.stamina -= int(self.collide_hit.damage*0.5)
        ...
        for stair in stair_objects:
            if self.rect.colliderect( stair.rect ):
                # Top
                if self.rect.y <= stair.rect.y -(stair.rect.height/2):
                    self.__gravity_current = 0
                    self.__air_count = 0
                    if not self.rect.y == stair.rect.y -self.rect.height:
                        self.rect.y = stair.rect.y -self.rect.height
                    collided_side = 'bottom'
...
class hit_object(pygame.sprite.Sprite):
    def __init__(self, size=grid_square, position=(0,0), damage=4, destroy_solids=False):
        ...
        self.damage = damage
        self.destroy_solids = destroy_solids
        layer_all_sprites.add(self, layer=2)
        update_objects.add(self)
...
class Box(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.damage_effect == True:
            ...
            #self.__gravity_current = random.choice( [0, -self.rect.height/8] )
            collide_and_move(
                obj=self, 
...
class Stair(pygame.sprite.Sprite):
    def __init__(self, position=[0,0], size=grid_square, direction='left'):
        ...
        size_stair = [size/2, size/2]
        
        pos_1 = [ position[0], position[1] ]
        if multipler == 0:
            pos_1[0] += size_stair[0]
        collide_1 = pygame.sprite.Sprite()
        collide_1.surf = pygame.Surface( size_stair, pygame.SRCALPHA)
        collide_1.surf.fill( generic_colors('red', self.transparency_collide) )
        collide_1.rect = collide_1.surf.get_rect( topleft=(pos_1) )
        stair_objects.add( collide_1 )
        layer_all_sprites.add( collide_1, layer=1 )

        pos_2 = [position[0]+(size_stair[0]*multipler), position[1]+size_stair[1] ]
        collide_2 = pygame.sprite.Sprite()
        collide_2.surf = pygame.Surface( size_stair, pygame.SRCALPHA )
        collide_2.surf.fill( generic_colors('blue', self.transparency_collide) )
        collide_2.rect = collide_2.surf.get_rect( topleft=pos_2 )
        stair_objects.add( collide_2 )
        layer_all_sprites.add( collide_2, layer=1 )
        
        pos_3 = [pos_1[0], pos_1[1]+size_stair[1]]
        collide_3 = pygame.sprite.Sprite()
        collide_3.surf = pygame.Surface( size_stair, pygame.SRCALPHA )
        collide_3.surf.fill( generic_colors('green', self.transparency_collide) )
        collide_3.rect = collide_3.surf.get_rect( topleft=pos_3 )
        stair_objects.add( collide_3 )
        layer_all_sprites.add( collide_3, layer=1 )
        
        if direction == 'left':
            Stone( size=size, position=(position[0]-size, position[1]) )
        else:
            Stone( size=size, position=(position[0]+size, position[1]) )
    
    
    def update(self):
        collide_1.surf.set_alpha( self.transparency_collide )
        collide_2.surf.set_alpha( self.transparency_collide )
        collide_3.surf.set_alpha( self.transparency_collide )
...
stair_objects = pygame.sprite.Group()
